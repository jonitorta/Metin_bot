...
def checar_muerte():
    ...
    vision_muerte = Vision('imagenes\muerte.png')



    screenshot_muerte = wincap.get_screenshot()
    rectangles_muerte = vision_muerte.find(screenshot_muerte, 0.80)
...
def buscar_metin(historial=0):
    vision_limestone = Vision("imagenes\jeon.png")
...
#------------------------------------ 
    ...
    if i %100==0:
        ...
        time.sleep(0.1)
        cancelar_habilidad("4")    
#-------------------------------------    
    punto=buscar_metin(historial)
    if punto[0] ==0:
        pydirectinput.press("z")    
        time.sleep(0.7)
        pydirectinput.press("space",11)
        pydirectinput.press("z")
        historial=punto[1]
    elif punto[0] ==2:
        pydirectinput.keyDown("s")
